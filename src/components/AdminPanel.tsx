...
import { Save, RotateCcw, X, DollarSign, Edit3, Plus, Trash2, Check, AlertCircle, Globe, Languages, ArrowUp, ArrowDown, GripVertical } from 'lucide-react';
import { RoutePrice, getPrices, savePrices, resetPrices, updateRoute, deleteRoute, addRoute, initializeDefaultData, reorderRoutes } from '../utils/firebaseStorage';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ isOpen, onClose }) => {
  ...
  const moveRoute = async (routeId: string, direction: 'up' | 'down') => {
    const currentIndex = prices.findIndex(r => r.id === routeId);
    ...
    [newPrices[currentIndex], newPrices[newIndex]] = [newPrices[newIndex], newPrices[currentIndex]];
    
    setPrices(newPrices);
    
    setLoading(true);
    const success = await reorderRoutes(newPrices);
    if (success) {
      window.dispatchEvent(new Event('pricesUpdated'));
      showNotification('success', 'Redosled je uspešno ažuriran!');
    } else {
      showNotification('error', 'Greška pri ažuriranju redosleda!');
      await loadPrices(); // Reload on error
    }
    setLoading(false);
  };
          ...
          <div className="p-6">
            ...
            {showAddForm && (
              <div className="bg-lime-50 rounded-2xl p-6 mb-6">
                ...
                {/* Multi-language price descriptions */}
                <div className="mb-6">
                  <h5 className="font-bold text-slate-800 mb-3">Opisi cena:</h5>
                  <div className="grid grid-cols-1 gap-4">
                    {languages.map((lang) => (
                      <div key={lang.code}>
                        ...
                        <input
                          type="text"
                          value={newRoute.priceDescriptions[lang.code]}
                          onChange={(e) => setNewRoute(prev => ({
                            ...prev,
                            priceDescriptions: { ...prev.priceDescriptions, [lang.code]: e.target.value }
                          }))}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-lime-400 focus:border-transparent"
                          placeholder={
                            lang.code === 'sr' ? 'po vozilu (cena je do 3 osobe, za četvrtu odraslu osobu je +1000rsd na cenu, deca nemaju doplatu)' :
                            lang.code === 'en' ? 'per vehicle (price is for up to 3 people, 4th adult person +1000rsd to the price, children have no surcharge)' :
                            'за автомобиль (цена до 3 человек, за 4-го взрослого +1000 динаров к цене, дети без доплаты)'
                          }
                        />
                      </div>
                    ))}
                ...
                <div key={route.id} className="bg-slate-50 rounded-2xl p-6 relative">
                  ...
                  <div className="absolute top-3 left-3 flex flex-col gap-1">
                    <button
                      ...
                      className="bg-lime-400 text-slate-800 p-1 rounded hover:bg-lime-300 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      title="Pomeri naviše"
                    >
                    ...
                    <button
                      ...
                      disabled={index === prices.length - 1 || loading}
                      className="bg-lime-400 text-slate-800 p-1 rounded hover:bg-lime-300 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      title="Pomeri naniže"
                    >
                  ...
                  <div className="flex items-center mb-4 pl-12 pr-8">
                  ...
                  {editingId === route.id && editingData ? (
                    <div className="space-y-3">
                      ...
                      <div>
                        <label className="block text-xs font-medium text-slate-600 mb-1">Opis cene ({selectedLanguage})</label>
                        <input
                          type="text"
                          value={editingData.priceDescriptions?.[selectedLanguage] || ''}
                          onChange={(e) => setEditingData(prev => prev ? { 
                            ...prev, 
                            priceDescriptions: { 
                              ...prev.priceDescriptions,
                    ...
                    <>
                      ...
                      <div className="text-xs text-slate-600 font-medium mb-2">
                        ID: {route.id} | Redosled: {route.order || index + 1}
                      </div>
                      <div className="text-xs text-slate-600 font-medium mb-2">
                        {route.priceDescriptions?.[selectedLanguage] || route.priceDescription || 'po vozilu (do 3 osobe + 1500 po osobi)'}
                      </div>
            ...
            <div className="mt-8 p-4 bg-lime-50 rounded-xl">
              ...
              <ul className="text-sm text-slate-600 space-y-1">
                ...
                <li>• Možete editovati nazive i opise cena za svaki jezik posebno</li>
                <li>• Koristite strelice za promenu redosleda transfera</li>
